...
RESTful API для маркетплейса скинов CS2/CS:GO, построенный на NestJS с JWT авторизацией.
...
API использует JWT (JSON Web Token) для авторизации пользователей.

### Быстрый старт

1. **Регистрация**: `POST /api/auth/register`
2. **Получаете токен** в ответе
3. **Используете токен** в заголовке `Authorization: Bearer YOUR_TOKEN`

### Доступные эндпоинты авторизации
...
// 2. Использование токена для защищенных запросов
...
- [JWT_AUTH_GUIDE.md](./JWT_AUTH_GUIDE.md) - Полное руководство по авторизации
- [AUTH_API.md](./AUTH_API.md) - Детальная документация API
- [api-requests.http](./api-requests.http) - Примеры запросов для тестирования
